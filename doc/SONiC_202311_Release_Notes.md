...
| ***Build improvements changes*** | The idea is to split the build_debian.sh into two stages with stage 1 to do all the needed preparation (debootstrap, apt-install, etc.) and save the rootfs into a temporary squashfs file. And stage 2 to continue the build from the point where the previous step finished (via loading the temporary squashfs as rootfs). | [1413](https://github.com/sonic-net/SONiC/issues/1413) | Alpha  |
